...
class MoEDirectMicroKernel:
    ...
    def _block_dot_hfma2_pair_regs_for_math(
        ...
        When fast_math is active, uses the fused dual-output inline-PTX
        fp4_dot8_dual_sum (one asm block, 2-way ILP). Otherwise falls back
        to two sequential fp4_dot8_sum_f32acc calls.
        """
        if cutlass.const_expr(self.fast_math):
